...
void PostfixEntrypointNames(ModuleOp &module) {
  module->walk([&](KrnlEntryPointOp entryOp) -> WalkResult {
    ...
    entryPointFunc.setSymName(StringRef(
        onnx_mlir::LLVMBuilder::SymbolPostfix(module, entryPointFuncName)));
...
void genSignatureFunction(ModuleOp &module,
  ...
  OpBuilder b(context);
  onnx_mlir::MultiDialectBuilder<onnx_mlir::LLVMBuilder> create(b, loc);

  // Common information.
  Type i8Type = IntegerType::get(context, 8);
  Type i32Type = IntegerType::get(context, 32);
  Type i64Type = IntegerType::get(context, 64);
  Type i64PtrTy = onnx_mlir::krnl::getPointerType(context, i64Type);
  Type i8PtrTy = onnx_mlir::krnl::getPointerType(context, i8Type);
  Type i8PtrPtrTy = onnx_mlir::krnl::getPointerType(context, i8PtrTy);
  ...
  { // Fill the initializer with pointers to entry point constants.
    ...
    for (const LLVM::GlobalOp &globalOp : entryGlobalOps) {
      Value strAddr = onnx_mlir::krnl::getPtrToGlobalString(globalOp, loc, b);
      lastValue =
    ...
    for (uint64_t j = 0; j < numOfEntryPoints; ++j) {
      ...
      create.llvm.ifThenElse(/*cond=*/
          [&](LLVMBuilder &createLLVM) {
            ...
            Value entryI8Ptr =
                onnx_mlir::krnl::getPtrToGlobalString(globalEntryPoint, loc, b);
            // Compare it with the user's entry point name.
            FlatSymbolRefAttr StrncmpRef =
                onnx_mlir::krnl::getOrInsertStrncmp(b, module);
...
bool extractConstantsToFile(ModuleOp &module, std::string filepath,
  ...
  Type llvmI8PtrTy = onnx_mlir::krnl::getPointerType(context, llvmI8Ty);
  ...
  module.walk([&](KrnlGlobalOp op) {
    ...
    ArrayRef<char> rawData = onnx_mlir::krnl::getRawData(op);
    if (rawData.empty())
      return WalkResult::advance();
...
void loadConstantsFromFile(ModuleOp &module,
  ...
  Type llvmI8PtrTy = onnx_mlir::krnl::getPointerType(ctx, llvmI8Ty);
  ...
  if (calledByEntryPoint) {
    Operation *firstEntryPointOp =
        onnx_mlir::krnl::getFirstEntryOpInBlock(module, entryGlobalOps);
    ...
    bool zOS = onnx_mlir::krnl::isZOS(module);
    for (auto entryGlobalOp : entryGlobalOps) {
      ...
      entryName = (zOS) ? onnx_mlir::krnl::e2a_s(entryName) : entryName;
      ...
      FlatSymbolRefAttr loadAllConstantsRef =
          create.llvm.getOrInsertSymbolRef(module,
              onnx_mlir::LLVMBuilder::SymbolPostfix(
                  module, loadAllConstantsFuncName),
              llvmVoidTy, {},
              /*isVarArg=*/false);
  ...
  std::string fnameSymbol = onnx_mlir::LLVMBuilder::SymbolPostfix(
      module, EXTERNAL_CONSTANT_PREFIX + "filename");
  ...
  Value fnameI8Ptr =
      onnx_mlir::krnl::getPtrToGlobalString(fnameGlobalOp, loc, b);
  // Get the file size.
  std::string fsizeSymbol = onnx_mlir::LLVMBuilder::SymbolPostfix(
      module, EXTERNAL_CONSTANT_PREFIX + "filesize");
  ...
  std::string isleSymbol = onnx_mlir::LLVMBuilder::SymbolPostfix(
      module, EXTERNAL_CONSTANT_PREFIX + "isLE");
  ...
  std::string packedSymbol = onnx_mlir::LLVMBuilder::SymbolPostfix(
...
void populateKrnlToLLVMConversion(LLVMTypeConverter &typeConverter,
  ...
  onnx_mlir::krnl::populateLoweringKrnlEntryPointOpPattern(typeConverter,
      patterns, ctx, outputOMTensorOwnerships, singleEntryPoint, entryGlobalOps,
      inSigGlobalOps, outSigGlobalOps, inputMemRefTypes, outputMemRefTypes,
      verifyInputTensors);
  onnx_mlir::krnl::populateLoweringKrnlCallOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlFindIndexOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlGlobalOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlInstrumentOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlMemcpyOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlPrintOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlPrintTensorOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlVectorTypeCastOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlRandomNormalOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlStrlenOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlUnaryMathOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlStrncmpOpPattern(
      typeConverter, patterns, ctx);
  onnx_mlir::krnl::populateLoweringKrnlNoneOpPattern(
      typeConverter, patterns, ctx);
}
