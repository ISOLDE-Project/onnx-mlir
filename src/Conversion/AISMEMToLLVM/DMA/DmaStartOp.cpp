...
#define DEBUG_TYPE "AISMEMToLLVM_DmaStartOp"

using namespace mlir;


namespace {
struct DmaStartOpLowering : public ConvertToLLVMPattern {
  using theOperation = memref::DmaStartOp;
  using theAdaptor = memref::DmaStartOpAdaptor;

  explicit DmaStartOpLowering(
...
private:
  inline static std::string funcName = "_dma_transfer";
};
} // namespace
namespace spade {
void populateMemrefDmaStartOpPattern(LLVMTypeConverter &typeConverter,
    RewritePatternSet &patterns, MLIRContext *ctx) {
  patterns.insert<DmaStartOpLowering>(typeConverter, ctx);
}
} // namespace spade